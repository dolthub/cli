...
func NewCmdRoot(f *cmdutil.Factory) *cobra.Command {
	...
	cmd.AddCommand(auth.NewCmdAuth(f), branch.NewCmdBranch(f), browse.NewCmdBrowse(f, nil), completion.NewCmdCompletion(), configcmd.NewCmdConfig(f), db.NewCmdDB(f), operation.NewCmdOperation(f), version.NewCmdVersion(f, nil))
	return cmd
}
