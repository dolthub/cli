...
func NewCmdRoot(f *cmdutil.Factory) *cobra.Command {
	...
	cmd.AddCommand(api.NewCmdAPI(f, nil), auth.NewCmdAuth(f), browse.NewCmdBrowse(f, nil), completion.NewCmdCompletion(), configcmd.NewCmdConfig(f), db.NewCmdDB(f), operation.NewCmdOperation(f), pr.NewCmdPR(f), release.NewCmdRelease(f), version.NewCmdVersion(f, nil))
	return cmd
}
