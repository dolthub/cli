...
func NewCmdClose(f *cmdutil.Factory, runF func(context.Context, *Options) error) *cobra.Command {
	o := &Options{IO: f.IO, ResolveRepository: f.ResolveRepository, APIClientForHost: f.APIClientForHost}
	if runF == nil {
		runF = closeRun
	}
	c := &cobra.Command{Use: "close NUMBER", Short: "Close a pull request", Args: func(c *cobra.Command, args []string) error {
	...
	return cmdutil.WithDocs(c, "dh pr close 1 --db OWNER/people", cmdutil.DocMetadata{
...
func closeRun(ctx context.Context, o *Options) error {
	return updateState(ctx, o, dolthub.PullStateClosed)
}
func updateState(ctx context.Context, o *Options, state dolthub.PullState) error {
