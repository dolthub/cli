...
const referencePage = "commands/README.md"
...
func Generate(root *cobra.Command, info buildinfo.Info) (Bundle, error) {
	...
	walk = func(cmd *cobra.Command) error {
		...
		e := entry{command: cmd, item: Command{Path: cmd.CommandPath(), Summary: cmd.Short, Page: referencePage, Anchor: anchor, Aliases: aliases}}
		if cmd.Runnable() {
	...
	const title = "DoltHub CLI command reference"
	const description = "Commands, flags, examples, and output formats for dh."
	...
	renderCommand(&out, entries[0], entries)
	...
	pageData := append(bytes.TrimRight(out.Bytes(), "\n"), '\n')
	hash := sha256.Sum256(pageData)
	bundle[referencePage] = pageData
	manifest.Pages = []Page{{Filename: referencePage, Title: title, Description: description, Route: BaseRoute, SHA256: hex.EncodeToString(hash[:])}}
